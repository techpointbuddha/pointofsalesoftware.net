            ...
            <div class="box box-primary">
                <div class="box-body">
                    ...
                    <div class="width-50 text-center f-left">
                        ...
                        @if(!empty($pos_settings['square_public_key']) && !empty($pos_settings['square_secret_key']))
                            @php
                                $code = strtolower($business_details->currency_code);
                                $mode = explode('-', $pos_settings['square_public_key']);
                                if (@$mode[0] !== "sandbox") $mode = \Square\Environment::PRODUCTION;
                                else $mode = \Square\Environment::SANDBOX;
                            @endphp

                            <script src="https://{{$mode === \Square\Environment::SANDBOX ? "sandbox." : ""}}web.squarecdn.com/v1/square.js"></script>
                            <form id='square-form' action="{{route('confirm_payment', ['id' => $transaction->id])}}" method="POST">
                                {{ csrf_field() }}
                                <input type="hidden" name="gateway" value="square" />
                                <input type='hidden' name='square_payment_token' />
                                <div id='payment-status-container'></div>
                                <div id='square-card-container'></div>
                                <button id="card-button" class='btn btn-primary' type="button">Pay with Square</button>
                            </form>
                            <script type="module">
                                const payments = Square.payments('{{$pos_settings['square_public_key']}}', '{{$pos_settings['square_secret_key']}}');
                                ...
                                cardButton.addEventListener('click', async () => {
                                    ...
                                    try {
                                        ...
                                        if (result.status === 'OK') {
                                            ...
                                            $("[name='square_payment_token']").val(result.token);
                                            $('#square-form').submit();
                                        } else {
