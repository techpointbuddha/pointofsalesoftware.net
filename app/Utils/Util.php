...
use Modules\Crm\Notifications\OrderRequestNotification;
...
class Util
{
    ...
    public function parseNotifications($notifications)
    {
        ...
        foreach ($notifications as $notification) {
            ...
            }elseif ( $notification->type == "Modules\Crm\Notifications\OrderRequestNotification"){
                $icon_class = "fas fa-list bg-warning";
                $notifications_data[] = [
                    'msg' => "You have a new Sale order Request with reference number ".$data['invoice_no']." sent by ".$data['user_name'],
                    'icon_class' => $icon_class,
                    'link' => route('sales-order.index'),
                    'read_at' => $notification->read_at,
                    'created_at' => $notification->created_at->diffForHumans()
                ];

            }elseif ( $notification->type == "App\Notifications\NewOrderNotify"){
                $icon_class = "fas fa-shopping-bag bg-success";
                $notifications_data[] = [
                    'msg' => "You have one Order Received from Website,Order No : ".$data['order_number'],
                    'icon_class' => $icon_class,
                    'link' => route('sells.index'),
                    'read_at' => $notification->read_at,
                    'created_at' => $notification->created_at->diffForHumans()
                ];
